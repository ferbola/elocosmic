<?php
  ...
  if ($login['id'] == null){
    ...
    $login_id = $login['id'];
    $servicoAdd = mysqli_query($mysqli, "INSERT INTO servico (titulo, valor , elo_inicial, elo_final) VALUES ('DuoBoost Valorant', '$valor',  '$elo_inicial', '$elo_final')");
    ...
    $servico = mysqli_query($mysqli, "SELECT id FROM servico WHERE valor = '$valor' AND elo_inicial = '$elo_inicial' AND elo_final = '$elo_final' AND titulo = 'DuoBoost Valorant'");
...
<body data-rsssl="1" class="class=" page-template-default="" page="" page-id-369="" woocommerce-no-js="" elementor-default="" elementor-kit-8""="">
    ...
    <section class="service-area-v3 pt-120 pb-120" aria-hidden="false" id="duoboost">
            <div class="container">
                ...
                <form action="ValorantDuoBoost.php" method="post" id="enviareloboost">
                <div class="row">
     ...
     jQuery('#dd').change(function(){
      ...
      jQuery('#dt').change(function(){
        ...
        opt = jQuery(this).children("option:selected").attr('id');
         if((opt == 'ascendente') || (opt == 'radiant') || (opt == 'immortal')) {
            jQuery('#dd').empty()
            .append('<option selected="selected">1</option><option>2</option><option>3</option>');
         } else {
            jQuery('#dd').empty()
  ...
  <div class="form-group row">
    ...
    <div class="col-8">
      ...
      <h6> No momento só aceitamos PIX.</h6>
                        ...
                        <button name="compra" type="submit" class="main-btn" style="width: 100%;" id="linkativo">Pagar</button></div>
                </div>
                </form>
            </div>
        </section>
